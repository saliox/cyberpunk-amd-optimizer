...
local function coopReadIn()
    pcall(function()
        ...
        -- main (hors relais, qui borne déjà). Le mod ne fait pas confiance
        -- aveugle au fichier.
...
function Coop.sync(dt)
    ...
    if not Coop.inbReceived then
        ...
        -- ts figé > coopStale = relais injoignable. Détecté MÊME à 1 seul pair :
        -- sinon un coop_in gelé (peerCount==1) bloquerait la vague jusqu'au timeout.
