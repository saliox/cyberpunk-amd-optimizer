...
async function main() {
  ...
  startRelay(['--host', '--port', String(PORT), '--dir', hostDir], 'host');
  await sleep(400);
  startRelay(['--join', '127.0.0.1', '--port', String(PORT), '--dir', c1], 'c1');
  startRelay(['--join', '127.0.0.1', '--port', String(PORT), '--dir', c2], 'c2');
